...
// ---------------------------------------------------------------------------
// Runtime detection: SPCS (OAuth token on disk) vs. local dev (PAT env var)
// ---------------------------------------------------------------------------
const SPCS_TOKEN_PATH = '/snowflake/session/token';
const IS_SPCS = fs.existsSync(SPCS_TOKEN_PATH);

let SNOWFLAKE_ACCOUNT, BASE_URL;
...
function getAuthHeaders(role) {
  ...
  if (IS_SPCS) {
    headers['Authorization'] = `Bearer ${getSpcsToken()}`;
...
if (IS_SPCS) {
  ...
  BASE_URL = `https://${SNOWFLAKE_HOST}`;
  console.log('Running in SPCS mode');
} else {
  SNOWFLAKE_ACCOUNT = process.env.SNOWFLAKE_ACCOUNT;
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', account: SNOWFLAKE_ACCOUNT, spcs: IS_SPCS });
});

// ---------------------------------------------------------------------------
// Static file serving (SPCS / production -- frontend build in ./public)
// ---------------------------------------------------------------------------
...
app.listen(PORT, () => {
  ...
  if (IS_SPCS) console.log('Mode: SPCS (OAuth token)');
  else console.log('Mode: Local dev (PAT)');
});
